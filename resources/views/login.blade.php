...
<div class="grid min-h-screen lg:grid-cols-2">
  ...
  <section class="relative hidden lg:flex flex-col justify-between brand-gradient p-12 text-white">
    ...
    <p class="text-xs text-white/50">&copy; {{ date('Y') }} EduXplore &middot; {{ $namaSekolah ?? 'Sekolah Menengah' }}</p>
  </section>
  ...
  <section class="flex items-center justify-center p-6 sm:p-10">
    <div class="w-full max-w-md">
      ...
      <form action="{{ route('proses_login') }}" method="POST" class="space-y-4" novalidate>
        @csrf
        <div>
          ...
          <input id="identity" type="text" name="identity" value="{{ old('identity') }}" required autofocus
                 class="field {{ $errors->has('identity') || $errors->has('login_error') ? 'field-error' : '' }}"
        ...
        <div>
          ...
          <div class="relative">
            <input id="password" type="password" name="password" required
                   class="field pr-16 {{ $errors->has('password') || $errors->has('login_error') ? 'field-error' : '' }}"
                   placeholder="••••••••">
      ...
      <p class="mt-6 text-center text-sm text-slate-500">
        ...
        <a href="{{ route('register_view') }}" class="font-bold text-brand hover:underline">Daftar di sini</a>
      </p>
